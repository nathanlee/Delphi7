...
procedure TLeaveListFam.FrameEnter(Sender: TObject);
var
  Year, Month, Day: Word;
begin
  OnEnter := nil;
...
procedure TLeaveListFam.FrameCanResize(Sender: TObject; var NewWidth,
  NewHeight: Integer; var Resize: Boolean);
begin
  ChgGridWidth(GridList);
end;

procedure TLeaveListFam.TBFindClick(Sender: TObject);
begin
...
procedure TLeaveListFam.TBAppoveClick(Sender: TObject);
begin
  if not OperRights(SYS_OPR_ID,'03007',SYS_VOUCH) then
  begin
...
procedure TLeaveListFam.GridListDrawColumnCell(Sender: TObject;
...
procedure TLeaveListFam.TBCancelClick(Sender: TObject);
begin
...
procedure TLeaveListFam.TBOffClick(Sender: TObject);
begin
  if not OperRights(SYS_OPR_ID,'03007',SYS_INSERT) then
  begin
    ShowMessage(MSG_USE_LIMITED);
    Exit;
  end;

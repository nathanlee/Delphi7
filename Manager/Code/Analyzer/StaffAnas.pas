...
procedure TStaffAnasFrm.FormCreate(Sender: TObject);
var
  Year, Month, Day: Word;
  I: integer;
begin
...
procedure TStaffAnasFrm.FormClose(Sender: TObject;
...
procedure TStaffAnasFrm.CBXTypeChange(Sender: TObject);
var
...
procedure TStaffAnasFrm.SBCustClick(Sender: TObject);
begin
...
procedure TStaffAnasFrm.btnSBAdd1Click(Sender: TObject);
begin
  if not OperRights(SYS_OPR_ID, '02021', SYS_SELECT) then
  begin
...
procedure TStaffAnasFrm.SaleStat;
var
...
begin
  ...
  begin
    Close;
    //DisableControls;
    Parameters.ParamByName('@Flag').Value := 0; //业务员销售统计;
...
procedure TStaffAnasFrm.Page0Change(Sender: TObject);
var
  vp, I: integer;
  Bar: TBarSeries;
...
procedure TStaffAnasFrm.Grid0TitleBtnClick(Sender: TObject; ACol: Integer;
  Column: TColumnEh);
begin
  GridTitleBtnClickSort(Sender, Column);
end;

procedure TStaffAnasFrm.SaleTrendStat;
var
...
procedure TStaffAnasFrm.Page1Change(Sender: TObject);
var
  I: integer;
begin
...
procedure TStaffAnasFrm.SaleAlarmStat;
var
  I, lEnd, lStart: integer;
begin
  ProcessData(True);
...
{业务员销售对比分析}
procedure TStaffAnasFrm.SaleParallStat;
var
  I, lEnd, lStart: integer;
begin
...
procedure TStaffAnasFrm.StaffParallAfterScroll(DataSet: TDataSet);
var
  i: integer;
begin
...
procedure TStaffAnasFrm.btnSBExt1Click(Sender: TObject);
begin
  Close;
end;

procedure TStaffAnasFrm.btnSBPrn1Click(Sender: TObject);
begin
  if not OperRights(SYS_OPR_ID, '02021', SYS_PRINT) then
  begin
    ShowMessage(MSG_USE_LIMITED);
    Exit;
  end;
...
procedure TStaffAnasFrm.SBExpClick(Sender: TObject);
var
  Grid: TDBGridEH;
begin
  if not OperRights(SYS_OPR_ID, '02021', SYS_PRINT) then
...
procedure TStaffAnasFrm.PageChange(Sender: TObject);
begin

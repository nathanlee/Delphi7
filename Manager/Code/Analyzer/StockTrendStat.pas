...
type
  TStockTrendStatFrm = class(TForm)
    Panel15: TPanel;
...
var
  StockTrendStatFrm: TStockTrendStatFrm;
...
procedure TStockTrendStatFrm.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  PrdtProc.Active := False;
  StockTrendStatFrm := nil;
  Action := caFree;
end;

procedure TStockTrendStatFrm.FormCreate(Sender: TObject);
var
  Year, Month, Day: Word;
  I: integer;
begin
...
procedure TStockTrendStatFrm.SBExtClick(Sender: TObject);
begin
  Close;
end;

procedure TStockTrendStatFrm.SBAddClick(Sender: TObject);
var
  I, lEnd, lStart: integer;
begin
  if not OperRights(SYS_OPR_ID, '02005', SYS_SELECT) then
  begin
...
procedure TStockTrendStatFrm.CBXTypeChange(Sender: TObject);
var
...
procedure TStockTrendStatFrm.SBCustClick(Sender: TObject);
begin
...
procedure TStockTrendStatFrm.GridTitleBtnClick(Sender: TObject; ACol: Integer;
  Column: TColumnEh);
begin
  GridTitleBtnClickSort(Sender, Column);
end;

procedure TStockTrendStatFrm.PageChange(Sender: TObject);
var
  I: integer;
begin
...
procedure TStockTrendStatFrm.SBPrnClick(Sender: TObject);
begin
  if not OperRights(SYS_OPR_ID, '02005', SYS_PRINT) then
  begin
    ShowMessage(MSG_USE_LIMITED);
    Exit;
  end;
...
procedure TStockTrendStatFrm.CbxTrendChange(Sender: TObject);
begin
  Page.ActivePageIndex := 0;
end;

procedure TStockTrendStatFrm.SBExpClick(Sender: TObject);
begin
  if not OperRights(SYS_OPR_ID, '02005', SYS_PRINT) then
  begin
    ShowMessage(MSG_USE_LIMITED);

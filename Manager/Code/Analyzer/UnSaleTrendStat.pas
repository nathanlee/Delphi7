...
type
  TUnSaleTrendStatFrm = class(TForm)
    Panel15: TPanel;
...
var
  UnSaleTrendStatFrm: TUnSaleTrendStatFrm;
...
procedure TUnSaleTrendStatFrm.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  PrdtProc.Active := False;
  UnSaleTrendStatFrm := nil;
  Action := caFree;
end;

procedure TUnSaleTrendStatFrm.FormCreate(Sender: TObject);
var
  Year, Month, Day: Word;
  I: integer;
begin
...
procedure TUnSaleTrendStatFrm.SBExtClick(Sender: TObject);
begin
  Close;
end;

procedure TUnSaleTrendStatFrm.SBAddClick(Sender: TObject);
var
  I, lEnd, lStart: integer;
begin
  if not OperRights(SYS_OPR_ID, '02015', SYS_SELECT) then
  begin
...
procedure TUnSaleTrendStatFrm.CBXTypeChange(Sender: TObject);
var
...
procedure TUnSaleTrendStatFrm.SBCustClick(Sender: TObject);
begin
...
procedure TUnSaleTrendStatFrm.GridTitleBtnClick(Sender: TObject; ACol: Integer;
  Column: TColumnEh);
begin
  GridTitleBtnClickSort(Sender, Column);
end;

procedure TUnSaleTrendStatFrm.PageChange(Sender: TObject);
var
  I: integer;
begin
...
procedure TUnSaleTrendStatFrm.SBPrnClick(Sender: TObject);
begin
  if not OperRights(SYS_OPR_ID, '02015', SYS_PRINT) then
  begin
    ShowMessage(MSG_USE_LIMITED);
    Exit;
  end;
...
procedure TUnSaleTrendStatFrm.CbxTrendChange(Sender: TObject);
begin
  Page.ActivePageIndex := 0;
end;

procedure TUnSaleTrendStatFrm.SBExpClick(Sender: TObject);
begin
  if not OperRights(SYS_OPR_ID, '02014', SYS_PRINT) then
  begin
    ShowMessage(MSG_USE_LIMITED);
    Exit;
  end;

...
procedure TCustSaleFrm.FormClose(Sender: TObject;
...
procedure TCustSaleFrm.FormCreate(Sender: TObject);
var
  Year, Month, Day: Word;
  I: integer;
begin
...
procedure TCustSaleFrm.SBExtClick(Sender: TObject);
begin
  Close;
end;

procedure TCustSaleFrm.SBAddClick(Sender: TObject);
var
  I, lEnd, lStart: integer;
begin
  if not OperRights(SYS_OPR_ID, '02002', SYS_SELECT) then
  begin
...
procedure TCustSaleFrm.CBXTypeChange(Sender: TObject);
var
...
procedure TCustSaleFrm.SBCustClick(Sender: TObject);
begin
...
procedure TCustSaleFrm.GridTitleBtnClick(Sender: TObject; ACol: Integer;
  Column: TColumnEh);
begin
  GridTitleBtnClickSort(Sender, Column);
end;

procedure TCustSaleFrm.PageChange(Sender: TObject);
var
  vp, I: integer;
  Bar: TBarSeries;
...
procedure TCustSaleFrm.SBPrnClick(Sender: TObject);
begin
  if not OperRights(SYS_OPR_ID, '02002', SYS_PRINT) then
  begin
    ShowMessage(MSG_USE_LIMITED);
    Exit;
  end;
...
procedure TCustSaleFrm.CbxSTypeChange(Sender: TObject);
begin
  if cbxsType.ItemIndex <> 3 then
    with cbxMonth do
    begin
...
procedure TCustSaleFrm.SBExpClick(Sender: TObject);
begin
  if not OperRights(SYS_OPR_ID, '02002', SYS_PRINT) then
  begin
    ShowMessage(MSG_USE_LIMITED);

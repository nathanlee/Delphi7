...
var
  PrdtUnStockFrm: TPrdtUnStockFrm;
...
procedure TPrdtUnStockFrm.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  StockStatProc.Active := False;
  PrdtUnStockFrm := nil;
  Action := caFree;
end;

procedure TPrdtUnStockFrm.FormCreate(Sender: TObject);
var
  Year, Month, Day: Word;
  I: integer;
begin
  ...
  Application.ProcessMessages;
  AddComBo(CbxCust, MainFrm.AdoTmp, True); //客户列表
  {添加商品类别}
  with MainFrm.AdoTmp do
  begin
    Close;
...
procedure TPrdtUnStockFrm.SBExtClick(Sender: TObject);
begin
  Close;
end;

procedure TPrdtUnStockFrm.SBSumClick(Sender: TObject);
var
  I, lEnd, lStart: integer;
begin
  if not OperRights(SYS_OPR_ID, '02003', SYS_SELECT) then
  begin
...
procedure TPrdtUnStockFrm.CBXTypeChange(Sender: TObject);
var
...
procedure TPrdtUnStockFrm.SBCustClick(Sender: TObject);
begin
...
procedure TPrdtUnStockFrm.GridTitleBtnClick(Sender: TObject; ACol: Integer;
  Column: TColumnEh);
begin
  GridTitleBtnClickSort(Sender, Column);
end;

procedure TPrdtUnStockFrm.PageChange(Sender: TObject);
var
  vp, I: integer;
  Bar: TBarSeries;
...
begin
  ...
  if (Page.ActivePageIndex = 1) and (not DS.DataSet.eof) then
  begin
...
procedure TPrdtUnStockFrm.SBPrnClick(Sender: TObject);
begin
  if not OperRights(SYS_OPR_ID, '02003', SYS_PRINT) then
  begin
    ShowMessage(MSG_USE_LIMITED);
    Exit;
  end;
...
procedure TPrdtUnStockFrm.CbxSTypeChange(Sender: TObject);
begin
  if cbxsType.ItemIndex <> 3 then
    with cbxMonth do
    begin
...
procedure TPrdtUnStockFrm.SBExpClick(Sender: TObject);
begin
  if not OperRights(SYS_OPR_ID, '02003', SYS_PRINT) then
  begin
    ShowMessage(MSG_USE_LIMITED);

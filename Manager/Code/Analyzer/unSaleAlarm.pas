...
var
  UnSaleAlarmFrm: TUnSaleAlarmFrm;
...
procedure TUnSaleAlarmFrm.FormClose(Sender: TObject;
...
procedure TUnSaleAlarmFrm.FormCreate(Sender: TObject);
var
  Year, Month, Day: Word;
  I: integer;
begin
...
procedure TUnSaleAlarmFrm.CBXTypeChange(Sender: TObject);
var
...
procedure TUnSaleAlarmFrm.SBTypeClick(Sender: TObject);
begin
...
procedure TUnSaleAlarmFrm.SBAddClick(Sender: TObject);
var
  I, lEnd, lStart: integer;
begin
  if not OperRights(SYS_OPR_ID, '02016', SYS_SELECT) then
  begin
...
procedure TUnSaleAlarmFrm.SBPrnClick(Sender: TObject);
begin
  if not OperRights(SYS_OPR_ID, '02016', SYS_PRINT) then
  begin
    ShowMessage(MSG_USE_LIMITED);
    Exit;
  end;
...
procedure TUnSaleAlarmFrm.SBExtClick(Sender: TObject);
begin
  Close;
end;

procedure TUnSaleAlarmFrm.GridTitleBtnClick(Sender: TObject; ACol: Integer;
  Column: TColumnEh);
begin
  GridTitleBtnClickSort(Sender, Column);
end;

procedure TUnSaleAlarmFrm.SBExpClick(Sender: TObject);
begin
  if not OperRights(SYS_OPR_ID, '02016', SYS_PRINT) then
  begin
    ShowMessage(MSG_USE_LIMITED);

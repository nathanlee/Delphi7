...
procedure TPrdtStockFrm.FormClose(Sender: TObject;
...
procedure TPrdtStockFrm.FormCreate(Sender: TObject);
var
  Year, Month, Day: Word;
  I: integer;
begin
...
procedure TPrdtStockFrm.SBExtClick(Sender: TObject);
begin
  Close;
end;

procedure TPrdtStockFrm.SBSumClick(Sender: TObject);
var
  I, lEnd, lStart: integer;
begin
  if not OperRights(SYS_OPR_ID, '02001', SYS_SELECT) then
  begin
...
procedure TPrdtStockFrm.CBXTypeChange(Sender: TObject);
var
...
procedure TPrdtStockFrm.SBCustClick(Sender: TObject);
begin
...
procedure TPrdtStockFrm.GridTitleBtnClick(Sender: TObject; ACol: Integer;
  Column: TColumnEh);
begin
  GridTitleBtnClickSort(Sender, Column);
end;

procedure TPrdtStockFrm.PageChange(Sender: TObject);
var
  vp, I: integer;
  Bar: TBarSeries;
...
procedure TPrdtStockFrm.SBPrnClick(Sender: TObject);
begin
  if not OperRights(SYS_OPR_ID, '02001', SYS_PRINT) then
  begin
    ShowMessage(MSG_USE_LIMITED);
    Exit;
  end;
...
procedure TPrdtStockFrm.CbxSTypeChange(Sender: TObject);
begin
  if cbxsType.ItemIndex <> 3 then
    with cbxMonth do
    begin
...
procedure TPrdtStockFrm.SBExpClick(Sender: TObject);
begin
  if not OperRights(SYS_OPR_ID, '02001', SYS_PRINT) then
  begin
    ShowMessage(MSG_USE_LIMITED);

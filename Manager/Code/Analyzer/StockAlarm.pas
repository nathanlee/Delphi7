...
procedure TStockAlarmFrm.FormClose(Sender: TObject;
...
procedure TStockAlarmFrm.FormCreate(Sender: TObject);
var
  Year, Month, Day: Word;
  I: integer;
begin
...
procedure TStockAlarmFrm.CBXTypeChange(Sender: TObject);
var
...
procedure TStockAlarmFrm.SBTypeClick(Sender: TObject);
begin
...
procedure TStockAlarmFrm.SBAddClick(Sender: TObject);
var
  I, lEnd, lStart: integer;
begin
  if not OperRights(SYS_OPR_ID, '02006', SYS_SELECT) then
  begin
...
procedure TStockAlarmFrm.SBPrnClick(Sender: TObject);
begin
  if not OperRights(SYS_OPR_ID, '02006', SYS_PRINT) then
  begin
    ShowMessage(MSG_USE_LIMITED);
    Exit;
  end;
...
procedure TStockAlarmFrm.SBExtClick(Sender: TObject);
begin
  Close;
end;

procedure TStockAlarmFrm.GridTitleBtnClick(Sender: TObject; ACol: Integer;
  Column: TColumnEh);
begin
  GridTitleBtnClickSort(Sender, Column);
end;

procedure TStockAlarmFrm.SBExpClick(Sender: TObject);
begin
  if not OperRights(SYS_OPR_ID, '02006', SYS_PRINT) then
  begin
    ShowMessage(MSG_USE_LIMITED);

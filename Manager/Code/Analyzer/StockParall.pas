...
var
  StockParallFrm: TStockParallFrm;
...
procedure TStockParallFrm.FormCreate(Sender: TObject);
var
  Year, Month, Day: Word;
  I: integer;
begin
...
procedure TStockParallFrm.FormClose(Sender: TObject;
...
procedure TStockParallFrm.SBExtClick(Sender: TObject);
begin
  Close;
end;

procedure TStockParallFrm.SBPrnClick(Sender: TObject);
begin
  if not OperRights(SYS_OPR_ID, '02007', SYS_PRINT) then
  begin
...
procedure TStockParallFrm.SBAddClick(Sender: TObject);
var
  I, lEnd, lStart: integer;
begin
  if not OperRights(SYS_OPR_ID, '02007', SYS_SELECT) then
  begin
    ShowMessage(MSG_USE_LIMITED);
    Exit;
  end;
...
procedure TStockParallFrm.SBCustClick(Sender: TObject);
begin
...
procedure TStockParallFrm.CBXTypeChange(Sender: TObject);
var
...
procedure TStockParallFrm.SaleParallAfterScroll(DataSet: TDataSet);
var
  i: integer;
begin
  Chart1.Series[0].Clear;
...
procedure TStockParallFrm.GridTitleBtnClick(Sender: TObject; ACol: Integer;
  Column: TColumnEh);
begin
  GridTitleBtnClickSort(Sender, Column);
end;

procedure TStockParallFrm.SBExpClick(Sender: TObject);
begin
  if not OperRights(SYS_OPR_ID, '02007', SYS_PRINT) then
  begin
    ShowMessage(MSG_USE_LIMITED);

...
type
  TCustSaleTrendStatFrm = class(TForm)
    Panel15: TPanel;
...
var
  CustSaleTrendStatFrm: TCustSaleTrendStatFrm;
...
procedure TCustSaleTrendStatFrm.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  PrdtProc.Active := False;
  CustSaleTrendStatFrm := nil;
  Action := caFree;
end;

procedure TCustSaleTrendStatFrm.FormCreate(Sender: TObject);
var
  Year, Month, Day: Word;
  I: integer;
begin
...
procedure TCustSaleTrendStatFrm.SBExtClick(Sender: TObject);
begin
  Close;
end;

procedure TCustSaleTrendStatFrm.SBAddClick(Sender: TObject);
var
  I, lEnd, lStart: integer;
begin
  if not OperRights(SYS_OPR_ID, '02009', SYS_SELECT) then
  begin
...
procedure TCustSaleTrendStatFrm.CBXTypeChange(Sender: TObject);
var
...
procedure TCustSaleTrendStatFrm.SBCustClick(Sender: TObject);
begin
...
procedure TCustSaleTrendStatFrm.GridTitleBtnClick(Sender: TObject; ACol: Integer;
  Column: TColumnEh);
begin
  GridTitleBtnClickSort(Sender, Column);
end;

procedure TCustSaleTrendStatFrm.PageChange(Sender: TObject);
var
  I: integer;
begin
...
procedure TCustSaleTrendStatFrm.SBPrnClick(Sender: TObject);
begin
  if not OperRights(SYS_OPR_ID, '02009', SYS_PRINT) then
  begin
    ShowMessage(MSG_USE_LIMITED);
    Exit;
  end;
...
procedure TCustSaleTrendStatFrm.CbxTrendChange(Sender: TObject);
begin
  Page.ActivePageIndex := 0;
end;

procedure TCustSaleTrendStatFrm.SBExpClick(Sender: TObject);
begin
  if not OperRights(SYS_OPR_ID, '02009', SYS_PRINT) then
  begin
    ShowMessage(MSG_USE_LIMITED);
